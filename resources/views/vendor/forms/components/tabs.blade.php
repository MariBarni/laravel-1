    ...
    >
        @foreach ($getChildComponentContainer()->getComponents() as $tab)
            <button
                ...
                x-bind:class="{
                    'text-gray-500 @if (config('forms.dark_mode')) @endif': tab !== '{{ $tab->getId() }}',
                    'filament-forms-tabs-component-button-active bg-white text-primary-600 @if (config('forms.dark_mode')) @endif': tab === '{{ $tab->getId() }}',
                }"
            >
                @if ($icon = $tab->getIcon())
                    <x-dynamic-component
                ...
                @if ($badge = $tab->getBadge())
                    <span
                        ...
                        x-bind:class="{
                            'bg-gray-200 @if (config('forms.dark_mode')) @endif': tab !== '{{ $tab->getId() }}',
